...
export function ComposerPicker({
  ...
  const show = (): void => {
    ...
    // model/Worker list directly instead of hiding Workers behind a Model row.
  ...
  const triggerLabel = selectedExternal !== undefined || effortLabel === undefined ? effectiveLabel : `${effectiveLabel} · ${effortLabel}`
        ...
        <>
          ...
          <div className={classNames(css.groups, 'scrollable')}>
            {visibleExternalTargets.length > 0 && (
              <section role="group" aria-label={externalTargetsLabel} className={css.group}>
                <div className={css.groupTitle}>{externalTargetsLabel}</div>
  ...
  return (
    <div className={classNames(css.root, embedded && css.embedded)} onKeyDown={onRootKeyDown}>
      <button
        ref={triggerRef}
        type="button"
      ...
      >
        <span className={css.triggerLabel}>{selectedExternal?.label ?? family?.name ?? modelLabel}</span>
        {selectedExternal === undefined && effortLabel !== undefined && <span className={css.triggerEffort}>{effortLabel}</span>}
